...
class Game {
    ...
    async endGame() {
        ...
        if (this.score > this.highScore) {
            ...
            localStorage.setItem('myGameHighScore', this.highScore);
        }
        ...
        try {
            // [체크] Render에서 'Web Service' 주소를 복사해서 여기에 넣으세요!
            // (뒤에 /submit은 유지하고, https:// 부분부터 .com까지만 수정)
            const SERVER_URL = "https://spaceship-adventure-server.onrender.com"; 
            ...
            const response = await fetch(`${SERVER_URL}/submit`, {
                ...
                // [체크] 서버가 어떤 게임인지 알 수 있게 'game_id'를 함께 보냅니다.
                // 다른 게임을 만들 때는 이 'spaceship_adventure'를 다른 이름으로 바꾸세요!
                body: JSON.stringify({
                    game_id: 'spaceship_adventure', 
                    score: this.score
                })
            });
